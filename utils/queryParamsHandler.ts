...
import { PaginationQuery, queryString, SearchQuery } from "../interfaces/queryParams";
...
class QueryParamsHandler {
  ...
  async pagination() {
    const documentsCount: number = await this.mongooseQuery.clone().countDocuments().exec();
